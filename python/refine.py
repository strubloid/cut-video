#!/usr/bin/env python3
"""Snap cut points to scene changes / audio zero crossings."""
...
def detect_scenes(input_file: str, threshold: float, duration: float) -> list[float]:
    cmd = [
    ...
    try:
        ...
        print("[refine] scene detection timed out, continuing without scene snap",
              file=sys.stderr)
        return []
...
def find_best_cut(data: np.ndarray, rate: int, time: float, window: float) -> float:
    n_samples = len(data)
...
def snap_to_scene(time: float, scenes: list[float], window: float) -> float | None:
...
def refine(
    ...
    *,
    ...
    print(f"[refine] audio: {rate} Hz, {duration:.2f}s, {n_samples} samples",
          file=sys.stderr)

    scenes = detect_scenes(input_path, threshold=0.4, duration=duration)
    print(f"[refine] detected {len(scenes)} scene changes in {input_path}",
          file=sys.stderr)

    kept = [s for s in plan["timeline"] if s.get("action") == "keep"]
    print(f"[refine] refining {len(kept)} keep-segments...", file=sys.stderr)

    refined_timeline: list[dict] = []
    scene_snapped = 0
    audio_snapped = 0
    unchanged = 0
    ...
    for seg in plan["timeline"]:
        ...
        if snapped_s is not None:
            ...
            new_s = round(find_best_cut(data, rate, orig_s, audio_window), 3)
        ...
        if snapped_e is not None:
            ...
            new_e = round(find_best_cut(data, rate, orig_e, audio_window), 3)
    ...
    print(f"[refine] scene-snap: {scene_snapped}, audio-snap: {audio_snapped}, "
...
def main() -> int:
    ...
    p.add_argument("--snap-window",  type=float, default=0.5)
    p.add_argument("--audio-window", type=float, default=0.2)
    args = p.parse_args()

    return refine(
        args.plan_json, args.refined_json, args.audio_wav, args.input_video,
